    ...
    <section class="portfolio__area pt-40 pb-40">
        ...
        >
          <div class="portfolio__masonary-btn-2 text-start mb-50">
            ...
            <div class="row gx-2 grid" v-if="items.length != 0 && !loading">
                <div
                    v-for="(it, i) in items"
                    :key="i"
...
onMounted(() => {
    loading.value = true;
    if (route.query.equipment_department) {
        let d = selectOptions.value.equipment_departments.find(
            (e) => e.title == route.query.equipment_department.toUpperCase()
        );
        search.value.equipment_department = d;
        loading.value = false;
    } else {
        search.value.equipment_department = {
            title: "SITT",
            value: 1,
        };
        loading.value = false;
...
watch([resEquipmentDepartment], () => {
    selectOptions.value.equipment_departments =
        resEquipmentDepartment.value.data;
});

useHead({
    title: "Sample Submission ศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยี คณะวิทยาศาสตร์ประยุกต์",
});
